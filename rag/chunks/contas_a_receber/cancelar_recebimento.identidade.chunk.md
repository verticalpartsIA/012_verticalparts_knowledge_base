---
source_path: "docs/omie/financeiro/contas_a_receber/cancelar_recebimento.md"
chunk_id: "CancelarRecebimento.identidade"
...
# Chunk CancelarRecebimento - identidade

Este chunk descreve o método `CancelarRecebimento` do serviço `LancamentoContaReceber` para recuperação RAG. Entrada oficial: `conta_receber_cancelar_recebimento`. Retorno oficial: `conta_receber_cancelar_recebimento_resposta`. Operação: `cancelamento_recebimento`.
...
Estratégia híbrida: usar busca lexical por `CancelarRecebimento`, `conta_receber_cancelar_recebimento`, código de lançamento, código de integração, baixa, conciliação, categoria e banco; combinar com busca vetorial por intenção de negócio. Reranking deve priorizar método exato, entidade conta a receber, status documentado oficialmente e compatibilidade com a operação solicitada.
