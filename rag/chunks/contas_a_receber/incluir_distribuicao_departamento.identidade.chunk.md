---
source_path: "docs/omie/financeiro/contas_a_receber/incluir_distribuicao_departamento.md"
chunk_id: "IncluirDistribuicaoDepartamento.identidade"
...
# Chunk IncluirDistribuicaoDepartamento - identidade

Este chunk descreve o método `IncluirDistribuicaoDepartamento` do serviço `LancamentoContaReceber` para recuperação RAG. Entrada oficial: `rateio_cadastro`. Retorno oficial: `conta_receber_cadastro_chave`. Operação: `inclusao_rateio_departamento`.
...
Estratégia híbrida: usar busca lexical por `IncluirDistribuicaoDepartamento`, `rateio_cadastro`, código de lançamento, código de integração, baixa, conciliação, categoria e banco; combinar com busca vetorial por intenção de negócio. Reranking deve priorizar método exato, entidade conta a receber, status documentado oficialmente e compatibilidade com a operação solicitada.
