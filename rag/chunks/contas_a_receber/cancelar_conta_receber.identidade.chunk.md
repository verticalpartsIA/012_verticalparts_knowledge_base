---
source_path: "docs/omie/financeiro/contas_a_receber/cancelar_conta_receber.md"
chunk_id: "CancelarContaReceber.identidade"
...
# Chunk CancelarContaReceber - identidade

Este chunk descreve o método `CancelarContaReceber` do serviço `LancamentoContaReceber` para recuperação RAG. Entrada oficial: `conta_receber_cadastro_chave`. Retorno oficial: `conta_receber_cadastro_response`. Operação: `cancelamento_boleto`.
...
Estratégia híbrida: usar busca lexical por `CancelarContaReceber`, `conta_receber_cadastro_chave`, código de lançamento, código de integração, baixa, conciliação, categoria e banco; combinar com busca vetorial por intenção de negócio. Reranking deve priorizar método exato, entidade conta a receber, status documentado oficialmente e compatibilidade com a operação solicitada.
