---
source_path: "docs/omie/financeiro/contas_a_receber/upsert_conta_receber_por_lote.md"
chunk_id: "UpsertContaReceberPorLote.identidade"
...
# Chunk UpsertContaReceberPorLote - identidade

Este chunk descreve o método `UpsertContaReceberPorLote` do serviço `LancamentoContaReceber` para recuperação RAG. Entrada oficial: `conta_receber_lote`. Retorno oficial: `conta_receber_lote_response`. Operação: `upsert_lote`.
...
Estratégia híbrida: usar busca lexical por `UpsertContaReceberPorLote`, `conta_receber_lote`, código de lançamento, código de integração, baixa, conciliação, categoria e banco; combinar com busca vetorial por intenção de negócio. Reranking deve priorizar método exato, entidade conta a receber, status documentado oficialmente e compatibilidade com a operação solicitada.
