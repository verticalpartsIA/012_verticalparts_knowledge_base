---
source_path: "docs/omie/financeiro/contas_a_receber/desconciliar_recebimento.md"
chunk_id: "DesconciliarRecebimento.identidade"
...
# Chunk DesconciliarRecebimento - identidade

Este chunk descreve o método `DesconciliarRecebimento` do serviço `LancamentoContaReceber` para recuperação RAG. Entrada oficial: `conta_receber_conciliar_request`. Retorno oficial: `conta_receber_conciliar_response`. Operação: `desconciliacao`.
...
Estratégia híbrida: usar busca lexical por `DesconciliarRecebimento`, `conta_receber_conciliar_request`, código de lançamento, código de integração, baixa, conciliação, categoria e banco; combinar com busca vetorial por intenção de negócio. Reranking deve priorizar método exato, entidade conta a receber, status documentado oficialmente e compatibilidade com a operação solicitada.
