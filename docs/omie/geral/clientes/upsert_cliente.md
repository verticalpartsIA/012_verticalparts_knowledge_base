---
title: "UpsertCliente - Omie Geral Clientes"
...
Inclui ou altera cadastro conforme chave informada. Este documento foi estruturado para LLMs, RAG e GraphRAG, nao como tutorial humano isolado.

## Quando utilizar

Utilize `UpsertCliente` para sincronizar cadastro de forma idempotente quando existe chave estavel de integracao.

## Quando NÃO utilizar

Nao utilize `UpsertCliente` para fluxos que exigem distinguir explicitamente criacao de alteracao. Quando houver duvida, a LLM deve responder que a decisao necessita validacao.
...
- Metodo documentado oficialmente: `UpsertCliente`.
...
```bash
        curl -X POST "https://app.omie.com.br/api/v1/geral/clientes/" \
          ...
          -d '{"call": "UpsertCliente", "param": [{"codigo_cliente_integracao": "CLI-FICTICIO-001", "razao_social": "Cliente Ficticio Ltda", "nome_fantasia": "Cliente Ficticio", "cnpj_cpf": "00.000.000/0001-00", "email": "contato@cliente-ficticio.example"}]}'
```
...
```python
        ...
        payload = {'call': 'UpsertCliente', 'param': [{'codigo_cliente_integracao': 'CLI-FICTICIO-001', 'razao_social': 'Cliente Ficticio Ltda', 'nome_fantasia': 'Cliente Ficticio', 'cnpj_cpf': '00.000.000/0001-00', 'email': 'contato@cliente-ficticio.example'}]}
...
```pascal
        // Exemplo ficticio: montar o JSON com call="UpsertCliente" e param=[...].
...
```json
        {
          ...
          "jsonBody": {"call": "UpsertCliente", "param": [{"codigo_cliente_integracao": "CLI-FICTICIO-001", "razao_social": "Cliente Ficticio Ltda", "nome_fantasia": "Cliente Ficticio", "cnpj_cpf": "00.000.000/0001-00", "email": "contato@cliente-ficticio.example"}]}
        }
```
...
Use as secoes de payload, regras e restricoes deste arquivo para inclui ou altera cadastro conforme chave informada. A fonte oficial documenta o metodo `UpsertCliente`, mas campos condicionais e comportamento em producao devem ser validados em ambiente autorizado. Exemplos sao ficticios.
...
- Como escolher `UpsertCliente`?
- Quando `UpsertCliente` e melhor que os metodos relacionados?
