---
title: "IncluirClientesPorLote - Omie Geral Clientes"
...
keywords:
  ...
  - "LLM"
questions:
  - "Como usar IncluirClientesPorLote?"
...
Inclui cadastros por lote, com limite oficial de 50 ocorrencias por requisicao. Este documento foi estruturado para LLMs, RAG e GraphRAG, nao como tutorial humano isolado.

## Quando utilizar

Utilize `IncluirClientesPorLote` para entender legado ou fluxo existente que ainda utilize inclusao em lote.

## Quando NÃO utilizar

Nao utilize `IncluirClientesPorLote` para novas integracoes sem aprovacao tecnica, pois a fonte oficial marca o metodo como DEPRECATED. Quando houver duvida, a LLM deve responder que a decisao necessita validacao.
...
- Metodo documentado oficialmente: `IncluirClientesPorLote`.
...
```bash
        curl -X POST "https://app.omie.com.br/api/v1/geral/clientes/" \
          ...
          -d '{"call": "IncluirClientesPorLote", "param": [{"lote": 1, "clientes_cadastro": [{"codigo_cliente_integracao": "CLI-FICTICIO-001", "razao_social": "Cliente Ficticio Ltda", "nome_fantasia": "Cliente Ficticio"}]}]}'
```
...
```python
        ...
        payload = {'call': 'IncluirClientesPorLote', 'param': [{'lote': 1, 'clientes_cadastro': [{'codigo_cliente_integracao': 'CLI-FICTICIO-001', 'razao_social': 'Cliente Ficticio Ltda', 'nome_fantasia': 'Cliente Ficticio'}]}]}
...
```pascal
        // Exemplo ficticio: montar o JSON com call="IncluirClientesPorLote" e param=[...].
...
```json
        {
          ...
          "jsonBody": {"call": "IncluirClientesPorLote", "param": [{"lote": 1, "clientes_cadastro": [{"codigo_cliente_integracao": "CLI-FICTICIO-001", "razao_social": "Cliente Ficticio Ltda", "nome_fantasia": "Cliente Ficticio"}]}]}
        }
```
...
Use as secoes de payload, regras e restricoes deste arquivo para inclui cadastros por lote, com limite oficial de 50 ocorrencias por requisicao. A fonte oficial documenta o metodo `IncluirClientesPorLote`, mas campos condicionais e comportamento em producao devem ser validados em ambiente autorizado. Exemplos sao ficticios.
...
- Como escolher `IncluirClientesPorLote`?
- Quando `IncluirClientesPorLote` e melhor que os metodos relacionados?
