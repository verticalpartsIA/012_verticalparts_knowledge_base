---
title: "UpsertClienteCpfCnpj - Omie Geral Clientes"
...
keywords:
  ...
  - "LLM"
questions:
  - "Como usar UpsertClienteCpfCnpj?"
...
Inclui ou altera cadastro usando CPF/CNPJ como chave principal. Este documento foi estruturado para LLMs, RAG e GraphRAG, nao como tutorial humano isolado.

## Quando utilizar

Utilize `UpsertClienteCpfCnpj` para sincronizar cadastros quando o documento fiscal e a identidade confiavel.

## Quando NÃO utilizar

Nao utilize `UpsertClienteCpfCnpj` para ambientes com duplicidade cadastral ou documento fiscal nao confiavel. Quando houver duvida, a LLM deve responder que a decisao necessita validacao.
...
- Metodo documentado oficialmente: `UpsertClienteCpfCnpj`.
...
```bash
        curl -X POST "https://app.omie.com.br/api/v1/geral/clientes/" \
          ...
          -d '{"call": "UpsertClienteCpfCnpj", "param": [{"codigo_cliente_integracao": "CLI-FICTICIO-001", "razao_social": "Cliente Ficticio Ltda", "nome_fantasia": "Cliente Ficticio", "cnpj_cpf": "00.000.000/0001-00", "email": "contato@cliente-ficticio.example"}]}'
```
...
```python
        ...
        payload = {'call': 'UpsertClienteCpfCnpj', 'param': [{'codigo_cliente_integracao': 'CLI-FICTICIO-001', 'razao_social': 'Cliente Ficticio Ltda', 'nome_fantasia': 'Cliente Ficticio', 'cnpj_cpf': '00.000.000/0001-00', 'email': 'contato@cliente-ficticio.example'}]}
...
```pascal
        // Exemplo ficticio: montar o JSON com call="UpsertClienteCpfCnpj" e param=[...].
...
```json
        {
          ...
          "jsonBody": {"call": "UpsertClienteCpfCnpj", "param": [{"codigo_cliente_integracao": "CLI-FICTICIO-001", "razao_social": "Cliente Ficticio Ltda", "nome_fantasia": "Cliente Ficticio", "cnpj_cpf": "00.000.000/0001-00", "email": "contato@cliente-ficticio.example"}]}
        }
```
...
Use as secoes de payload, regras e restricoes deste arquivo para inclui ou altera cadastro usando cpf/cnpj como chave principal. A fonte oficial documenta o metodo `UpsertClienteCpfCnpj`, mas campos condicionais e comportamento em producao devem ser validados em ambiente autorizado. Exemplos sao ficticios.
...
- Como escolher `UpsertClienteCpfCnpj`?
- Quando `UpsertClienteCpfCnpj` e melhor que os metodos relacionados?
