...
Use este metodo quando a operacao desejada corresponder ao metodo oficial `Listar` do servico `Pedidos de Venda`.
...
- Automatizar a operacao `Listar` no contexto do servico `Pedidos de Venda`.
...
Necessita validacao.
### Como Listar se relaciona com Pedidos de Venda?
