---
title: "ExcluirDistribuicaoDepartamento - Omie Financeiro Contas a Receber"
...
keywords:
  ...
  - RAG
questions:
  - "Como usar ExcluirDistribuicaoDepartamento?"
...
Exclui a distribuicao por departamento de uma conta a receber. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `exclusao_rateio_departamento` em contas a receber do Omie.
...
4. Executar a operação `ExcluirDistribuicaoDepartamento` com payload fictício em exemplos ou dados autorizados em produção.
...
- Tipo oficial de entrada: `conta_receber_cadastro_chave`.
- Tipo oficial de retorno: `conta_receber_cadastro_response`.
- Endpoint oficial: `https://app.omie.com.br/api/v1/financas/contareceber/`.

## Campos obrigatórios

- `codigo_lancamento_omie ou codigo_lancamento_integracao`
...
Retorno oficial: `conta_receber_cadastro_response`. A interpretação de códigos e mensagens deve ser validada em integração.
...
- Revisar campos obrigatórios do tipo `conta_receber_cadastro_chave`.
...
```json
{
  "call": "ExcluirDistribuicaoDepartamento",
  "param": [
    {
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "ExcluirDistribuicaoDepartamento", "param": [{"codigo_lancamento_omie": 123456789, "codigo_lancamento_integracao": "CR-FICTICIO-001"}]}'
```
...
payload = {'call': 'ExcluirDistribuicaoDepartamento', 'param': [{'codigo_lancamento_omie': 123456789, 'codigo_lancamento_integracao': 'CR-FICTICIO-001'}]}
...
var payload = new { call = "ExcluirDistribuicaoDepartamento", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="ExcluirDistribuicaoDepartamento".
```
...
```json
{
  ...
  "jsonBody": {"call": "ExcluirDistribuicaoDepartamento", "param": [{"codigo_lancamento_omie": 123456789, "codigo_lancamento_integracao": "CR-FICTICIO-001"}]}
}
```
...
O método `ExcluirDistribuicaoDepartamento` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `ExcluirDistribuicaoDepartamento`?
