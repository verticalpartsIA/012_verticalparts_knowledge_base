...
Lista contas a receber cadastradas. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `listagem` em contas a receber do Omie.
...
4. Executar a operação `ListarContasReceber` com payload fictício em exemplos ou dados autorizados em produção.
...
- Tipo oficial de entrada: `lcrListarRequest`.
- Tipo oficial de retorno: `lcrListarResponse`.
...
Retorno oficial: `lcrListarResponse`. A interpretação de códigos e mensagens deve ser validada em integração.
...
- Revisar campos obrigatórios do tipo `lcrListarRequest`.
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "ListarContasReceber", "param": [{"pagina": 1, "registros_por_pagina": 50, "apenas_importado_api": "N"}]}'
```
...
payload = {'call': 'ListarContasReceber', 'param': [{'pagina': 1, 'registros_por_pagina': 50, 'apenas_importado_api': 'N'}]}
...
var payload = new { call = "ListarContasReceber", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="ListarContasReceber".
```
...
```json
{
  ...
  "jsonBody": {"call": "ListarContasReceber", "param": [{"pagina": 1, "registros_por_pagina": 50, "apenas_importado_api": "N"}]}
}
```
...
O método `ListarContasReceber` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `ListarContasReceber`?
