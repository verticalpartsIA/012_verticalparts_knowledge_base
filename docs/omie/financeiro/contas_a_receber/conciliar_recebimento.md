---
title: "ConciliarRecebimento - Omie Financeiro Contas a Receber"
service: "LancamentoContaReceber"
domain: "omie.financeiro"
resource: "contas_a_receber"
method: "ConciliarRecebimento"
...
keywords:
  ...
  - RAG
questions:
  - "Como usar ConciliarRecebimento?"
...
Realiza a conciliacao do recebimento. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `conciliacao` em contas a receber do Omie.
...
4. Executar a operação `ConciliarRecebimento` com payload fictício em exemplos ou dados autorizados em produção.
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "ConciliarRecebimento", "param": [{"codigo_baixa": 123456789}]}'
```
...
payload = {'call': 'ConciliarRecebimento', 'param': [{'codigo_baixa': 123456789}]}
...
var payload = new { call = "ConciliarRecebimento", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="ConciliarRecebimento".
```
...
```json
{
  ...
  "jsonBody": {"call": "ConciliarRecebimento", "param": [{"codigo_baixa": 123456789}]}
}
```
...
O método `ConciliarRecebimento` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `ConciliarRecebimento`?
