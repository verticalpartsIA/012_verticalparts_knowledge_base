---
title: "AlterarDistribuicaoDepartamento - Omie Financeiro Contas a Receber"
service: "LancamentoContaReceber"
domain: "omie.financeiro"
resource: "contas_a_receber"
method: "AlterarDistribuicaoDepartamento"
...
keywords:
  ...
  - RAG
questions:
  - "Como usar AlterarDistribuicaoDepartamento?"
...
Altera a distribuicao por departamento de uma conta a receber. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `alteracao_rateio_departamento` em contas a receber do Omie.
...
4. Executar a operação `AlterarDistribuicaoDepartamento` com payload fictício em exemplos ou dados autorizados em produção.
...
- Tipo oficial de entrada: `rateio_cadastro`.
- Tipo oficial de retorno: `conta_receber_cadastro_chave`.
...
Retorno oficial: `conta_receber_cadastro_chave`. A interpretação de códigos e mensagens deve ser validada em integração.
...
- Revisar campos obrigatórios do tipo `rateio_cadastro`.
...
```json
{
  "call": "AlterarDistribuicaoDepartamento",
  "param": [
    {
      "codigo_lancamento_omie": 123456789,
      "distribuicao": [
{
  "cCodDep": "DEP-FICTICIO",
  "nPerDep": 100.0
}
      ]
    }
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "AlterarDistribuicaoDepartamento", "param": [{"codigo_lancamento_omie": 123456789, "distribuicao": [{"cCodDep": "DEP-FICTICIO", "nPerDep": 100.0}]}]}'
```
...
payload = {'call': 'AlterarDistribuicaoDepartamento', 'param': [{'codigo_lancamento_omie': 123456789, 'distribuicao': [{'cCodDep': 'DEP-FICTICIO', 'nPerDep': 100.0}]}]}
...
var payload = new { call = "AlterarDistribuicaoDepartamento", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="AlterarDistribuicaoDepartamento".
```
...
```json
{
  ...
  "jsonBody": {"call": "AlterarDistribuicaoDepartamento", "param": [{"codigo_lancamento_omie": 123456789, "distribuicao": [{"cCodDep": "DEP-FICTICIO", "nPerDep": 100.0}]}]}
}
```
...
O método `AlterarDistribuicaoDepartamento` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `AlterarDistribuicaoDepartamento`?
