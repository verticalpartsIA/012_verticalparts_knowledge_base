---
title: "ConsultarContaReceber - Omie Financeiro Contas a Receber"
...
keywords:
  ...
  - RAG
questions:
  - "Como usar ConsultarContaReceber?"
...
Consulta uma conta a receber por chave. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `consulta` em contas a receber do Omie.
...
4. Executar a operação `ConsultarContaReceber` com payload fictício em exemplos ou dados autorizados em produção.
...
- Tipo oficial de entrada: `lcrChave`.
- Tipo oficial de retorno: `conta_receber_cadastro`.
...
Retorno oficial: `conta_receber_cadastro`. A interpretação de códigos e mensagens deve ser validada em integração.
...
- Revisar campos obrigatórios do tipo `lcrChave`.
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "ConsultarContaReceber", "param": [{"codigo_lancamento_omie": 123456789, "codigo_lancamento_integracao": "CR-FICTICIO-001"}]}'
```
...
payload = {'call': 'ConsultarContaReceber', 'param': [{'codigo_lancamento_omie': 123456789, 'codigo_lancamento_integracao': 'CR-FICTICIO-001'}]}
...
var payload = new { call = "ConsultarContaReceber", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="ConsultarContaReceber".
```
...
```json
{
  ...
  "jsonBody": {"call": "ConsultarContaReceber", "param": [{"codigo_lancamento_omie": 123456789, "codigo_lancamento_integracao": "CR-FICTICIO-001"}]}
}
```
...
O método `ConsultarContaReceber` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `ConsultarContaReceber`?
