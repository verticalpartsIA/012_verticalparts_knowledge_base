---
title: "CancelarContaReceber - Omie Financeiro Contas a Receber"
service: "LancamentoContaReceber"
domain: "omie.financeiro"
resource: "contas_a_receber"
method: "CancelarContaReceber"
...
keywords:
  ...
  - RAG
questions:
  - "Como usar CancelarContaReceber?"
...
Cancela o boleto gerado de uma conta a receber. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `cancelamento_boleto` em contas a receber do Omie.
...
4. Executar a operação `CancelarContaReceber` com payload fictício em exemplos ou dados autorizados em produção.
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "CancelarContaReceber", "param": [{"codigo_lancamento_omie": 123456789, "codigo_lancamento_integracao": "CR-FICTICIO-001"}]}'
```
...
payload = {'call': 'CancelarContaReceber', 'param': [{'codigo_lancamento_omie': 123456789, 'codigo_lancamento_integracao': 'CR-FICTICIO-001'}]}
...
var payload = new { call = "CancelarContaReceber", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="CancelarContaReceber".
```
...
```json
{
  ...
  "jsonBody": {"call": "CancelarContaReceber", "param": [{"codigo_lancamento_omie": 123456789, "codigo_lancamento_integracao": "CR-FICTICIO-001"}]}
}
```
...
O método `CancelarContaReceber` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `CancelarContaReceber`?
