---
title: "CancelarRecebimento - Omie Financeiro Contas a Receber"
service: "LancamentoContaReceber"
domain: "omie.financeiro"
resource: "contas_a_receber"
method: "CancelarRecebimento"
...
Cancela uma baixa de recebimento de contas a receber. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `cancelamento_recebimento` em contas a receber do Omie.
...
4. Executar a operação `CancelarRecebimento` com payload fictício em exemplos ou dados autorizados em produção.
...
- Tipo oficial de entrada: `conta_receber_cancelar_recebimento`.
- Tipo oficial de retorno: `conta_receber_cancelar_recebimento_resposta`.
...
Retorno oficial: `conta_receber_cancelar_recebimento_resposta`. A interpretação de códigos e mensagens deve ser validada em integração.
...
- Revisar campos obrigatórios do tipo `conta_receber_cancelar_recebimento`.
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "CancelarRecebimento", "param": [{"codigo_baixa": 123456789}]}'
```
...
payload = {'call': 'CancelarRecebimento', 'param': [{'codigo_baixa': 123456789}]}
...
var payload = new { call = "CancelarRecebimento", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="CancelarRecebimento".
```
...
```json
{
  ...
  "jsonBody": {"call": "CancelarRecebimento", "param": [{"codigo_baixa": 123456789}]}
}
```
...
O método `CancelarRecebimento` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `CancelarRecebimento`?
