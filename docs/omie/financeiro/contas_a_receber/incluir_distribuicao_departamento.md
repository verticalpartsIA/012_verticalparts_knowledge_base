---
title: "IncluirDistribuicaoDepartamento - Omie Financeiro Contas a Receber"
service: "LancamentoContaReceber"
domain: "omie.financeiro"
resource: "contas_a_receber"
method: "IncluirDistribuicaoDepartamento"
...
keywords:
  ...
  - RAG
questions:
  - "Como usar IncluirDistribuicaoDepartamento?"
...
Inclui distribuicao por departamento em uma conta a receber. O conteúdo é preparado para LLMs, RAG e GraphRAG, com separação clara entre "Documentado oficialmente" e "Necessita validação".

## Quando utilizar

Utilize quando a intenção do usuário estiver ligada à operação `inclusao_rateio_departamento` em contas a receber do Omie.
...
4. Executar a operação `IncluirDistribuicaoDepartamento` com payload fictício em exemplos ou dados autorizados em produção.
...
```json
{
  "call": "IncluirDistribuicaoDepartamento",
  "param": [
    {
...
```bash
curl -X POST "https://app.omie.com.br/api/v1/financas/contareceber/" -H "Content-Type: application/json" -d '{"call": "IncluirDistribuicaoDepartamento", "param": [{"codigo_lancamento_omie": 123456789, "distribuicao": [{"cCodDep": "DEP-FICTICIO", "nPerDep": 100.0}]}]}'
```
...
payload = {'call': 'IncluirDistribuicaoDepartamento', 'param': [{'codigo_lancamento_omie': 123456789, 'distribuicao': [{'cCodDep': 'DEP-FICTICIO', 'nPerDep': 100.0}]}]}
...
var payload = new { call = "IncluirDistribuicaoDepartamento", param = new object[] { new { codigo_lancamento_integracao = "CR-FICTICIO-001" } } };
...
```pascal
// Exemplo ficticio: enviar POST para o endpoint oficial com call="IncluirDistribuicaoDepartamento".
```
...
```json
{
  ...
  "jsonBody": {"call": "IncluirDistribuicaoDepartamento", "param": [{"codigo_lancamento_omie": 123456789, "distribuicao": [{"cCodDep": "DEP-FICTICIO", "nPerDep": 100.0}]}]}
}
```
...
O método `IncluirDistribuicaoDepartamento` é documentado oficialmente na página do serviço LancamentoContaReceber. Detalhes operacionais, obrigatoriedade condicional e comportamento em produção necessitam validação em ambiente autorizado. Os exemplos deste repositório são fictícios.
...
- Como escolher `IncluirDistribuicaoDepartamento`?
